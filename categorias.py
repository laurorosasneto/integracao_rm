...
def main() -> int:
    ...
    missing = []
    if idx_codcol is None: missing.append("CODCOLIGADA")
    if idx_coligada is None: missing.append("COLIGADA")
    if idx_codperlet is None: missing.append("CODPERLET/PERIODO/CATPERIODO")
    if idx_idperlet is None: missing.append("IDPERLET")
    if idx_catmodalidade is None: missing.append("CATMODALIDADE")
